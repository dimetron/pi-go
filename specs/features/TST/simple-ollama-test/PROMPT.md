...
Create a working E2E test that runs `pi --model minimax-m3:cloud` against a real Ollama backend, has the agent explore
the codebase, and generates a `PI.md` project overview. The test must validate that session logs contain zero tool
errors and no skipped commands.
...
2. **Test script** at `scripts/test-ollama-e2e.sh` that:
   ...
   - Builds pi binary
   - Runs `pi --model minimax-m3:cloud --mode print` with a prompt to explore codebase and write PI.md
...
- The minimax-m3 model via Ollama may ignore the user prompt and output greeting text instead of following tool-use
  instructions
...
- Design: `specs/features/TST/simple-ollama-test/design.md`
- Plan: `specs/features/TST/simple-ollama-test/plan.md`
- Research: `specs/features/TST/simple-ollama-test/research/`
- Existing E2E tests: `internal/agent/e2e_test.go`, `internal/agent/e2e_enhanced_test.go`
- CLI print mode: `internal/cli/cli.go` (`runPrint`)
