...
- **`/plan <rough idea>`** — Interactive multi-turn PDD session that produces structured spec artifacts (`specs/features/{task_name}/`) culminating in a `PROMPT.md` ready for autonomous execution.
...
3. The LLM drives the multi-turn conversation following the SOP phases:
   - **Skeleton creation**: Create `specs/features/{task_name}/` with rough-idea.md, empty requirements.md, research/ directory
...
2. Command reads `specs/features/<spec-name>/PROMPT.md`
...
// 3. Creates specs/features/{task_name}/ skeleton via file writes
...
## Reference
- Design: `specs/features/<task_name>/design.md`
- Plan: `specs/features/<task_name>/plan.md`
- Requirements: `specs/features/<task_name>/requirements.md`
- Research: `specs/features/<task_name>/research/`
...
type runState struct {
    ...
    specDir      string       // e.g., "specs/features/SOP/plan-command-sop"
...
| `specs/features/{task_name}/` already exists | Show error, suggest different name or manual cleanup |
...
| `specs/features/<spec-name>/PROMPT.md` not found | Show error, list available specs |
...
### /plan Command
- Given user types `/plan add rate limiting to API`, when command executes, then `specs/features/add-rate-limiting-to-api/` directory is created with rough-idea.md
...
- Given `specs/features/<spec-name>/PROMPT.md` does not exist, when `/run` invoked, then error shown with list of available specs
