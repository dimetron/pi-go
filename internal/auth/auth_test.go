...
func TestAnthropicProvider_CompleteFlowWithAPIKeyExchange(t *testing.T) {
	// Simulate the full Anthropic OAuth flow:
	// 1. Exchange auth code for OAuth token (with raw_key)
	// 2. Exchange OAuth access token for API key via create_api_key endpoint
	var oauthCalled, apiKeyCalled bool
	tokenServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		oauthCalled = true
		var body map[string]string
		if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		...
		if body["redirect_uri"] != "http://localhost:53692/callback" {
			t.Errorf("wrong redirect_uri: %q", body["redirect_uri"])
		}
		if body["client_id"] != "9d1c250a-e61b-44d9-88ed-5944d1962f5e" {
			t.Errorf("wrong client_id: %q", body["client_id"])
		}
		if body["code_verifier"] == "" {
			t.Error("code_verifier should be sent")
		}
		// Token response with both access_token and raw_key (API key from org)
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(TokenResponse{
			AccessToken:  "oauth-access-token-123",
			RefreshToken: "refresh-token-456",
			ExpiresIn:    3600,
			RawKey:       "sk-ant-api03-test-api-key",
		})
	}))
	defer tokenServer.Close()
	...
	apiKeyServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		apiKeyCalled = true
		if auth := r.Header.Get("Authorization"); auth != "Bearer oauth-access-token-123" {
			t.Errorf("expected bearer auth, got %q", auth)
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"raw_key":"sk-ant-api03-claude-cli-key"}`))
	}))
	defer apiKeyServer.Close()

	// Build a provider matching the real Anthropic config
	prov := Provider{
		Name:              "anthropic-test",
	...
	result, err := CompleteManualCodeFlow(context.Background(), sess, "test-auth-code#test-state-xyz")
	if err != nil {
		t.Fatalf("CompleteManualCodeFlow error: %v", err)
	}
	if result.Err != nil {
	...
	if !oauthCalled {
		t.Error("OAuth token exchange was not called")
	}

	// API key exchange should have been called (because RawKey was in response)
	if !apiKeyCalled {
		t.Error("API key exchange was not called")
	}

	// The API key should be the minted one from create_api_key
	if result.APIKey != "sk-ant-api03-claude-cli-key" {
		t.Errorf("expected API key sk-ant-api03-claude-cli-key, got %q", result.APIKey)
	}
...
// TestAnthropicManualLoginURL_MatchesGoodExample pins the Anthropic manual-code
// authorization URL to the exact format emitted by Claude Code. Matching the
// good example byte-for-byte (same scheme/host/path, same client_id, same
// redirect_uri, same scope values in the canonical order, PKCE S256) is what
// lets the server accept the authorization request instead of rejecting it
// with "Invalid request format".
//
// Good example (from a successful Claude login session):
//
...
func TestAnthropicManualLoginURL_MatchesGoodExample(t *testing.T) {
	prov, ok := FindProvider("anthropic")
	if !ok {
		t.Fatal("anthropic provider not found")
	}
	...
	if u.Host != "claude.com" {
		t.Errorf("host = %q, want claude.com", u.Host)
	}
	if u.Path != "/cai/oauth/authorize" {
		t.Errorf("path = %q, want /cai/oauth/authorize", u.Path)
	}

	q := u.Query()

	// Same app IDs as the good example.
	if got, want := q.Get("client_id"), "9d1c250a-e61b-44d9-88ed-5944d1962f5e"; got != want {
		t.Errorf("client_id = %q, want %q", got, want)
	}
	if got, want := q.Get("redirect_uri"), "https://platform.claude.com/oauth/code/callback"; got != want {
		t.Errorf("redirect_uri = %q, want %q", got, want)
	}

	// Fixed/literal parameters.
	if got, want := q.Get("response_type"), "code"; got != want {
		t.Errorf("response_type = %q, want %q", got, want)
	}
	if got, want := q.Get("code"), "true"; got != want {
	...
	// Scope value (pre-encoding) must match the good example byte-for-byte,
	// including ordering (org:create_api_key first).
	wantScope := "org:create_api_key user:profile user:inference user:sessions:claude_code user:mcp_servers user:file_upload"
	if got := q.Get("scope"); got != wantScope {
		t.Errorf("scope = %q, want %q", got, wantScope)
	}

	// PKCE challenge and state are random per session, so only assert presence.
	if q.Get("code_challenge") == "" {
		t.Error("code_challenge must be present")
	}
	if q.Get("state") == "" {
		t.Error("state must be present")
	}

	// Required parameter set — no extras, no missing keys.
	...
	for k := range q {
		if !wantKeys[k] {
			t.Errorf("unexpected query parameter: %q", k)
		}
	}
	for k := range wantKeys {
		if _, ok := q[k]; !ok {
			t.Errorf("missing query parameter: %q", k)
		}
	}
}

func TestAnthropicProvider_ConfigValidation(t *testing.T) {
	// Verify the real Anthropic provider has all required fields
	prov, ok := FindProvider("anthropic")
	if !ok {
		t.Fatal("anthropic provider not found")
	}
	...
	if prov.APIKeyURL == "" {
		t.Error("APIKeyURL should not be empty (required to exchange OAuth token for API key)")
	}
	if prov.ClientID == "" {
		t.Error("ClientID should not be empty")
	}
	if !prov.ManualCode {
		t.Error("ManualCode should be true for Anthropic")
	}
	if !prov.TokenJSONBody {
		t.Error("TokenJSONBody should be true for Anthropic")
	}
	if prov.ManualRedirectURI == "" {
		t.Error("ManualRedirectURI should not be empty")
	}
	if prov.KeyPageURL == "" {
		t.Error("KeyPageURL should not be empty")
	}

	// Verify APIKeyURL points to Anthropic's create_api_key endpoint
	if !strings.Contains(prov.APIKeyURL, "console.anthropic.com") {
		t.Errorf("APIKeyURL should point to console.anthropic.com, got %q", prov.APIKeyURL)
	}
	if !strings.Contains(prov.APIKeyURL, "create_api_key") {
		t.Errorf("APIKeyURL should contain create_api_key path, got %q", prov.APIKeyURL)
	}

	// Verify scope includes org:create_api_key
	scope := strings.Join(prov.Scopes, " ")
	if !strings.Contains(scope, "org:create_api_key") {
		t.Errorf("scope should include org:create_api_key, got %q", scope)
	}
}
