...
func TestLoadBundledAgents(t *testing.T) {
	...
	// Expected 12 agents: explore, plan, designer, task, quick-task, worker, code-reviewer, spec-reviewer, memory-compressor, discovery, claude, gemini
	if len(agents) != 12 {
		t.Errorf("expected 12 bundled agents, got %d: %v", len(agents), agentNames(agents))
	}
...
func TestDiscoverAgents_Bundled(t *testing.T) {
	...
	// Should have 12 bundled agents
	if len(result.Bundled) != 12 {
		t.Errorf("expected 12 bundled agents, got %d", len(result.Bundled))
	}
...
func TestDiscoverAgents_Both(t *testing.T) {
	...
	if len(result.All) < 12 {
		t.Errorf("expected at least 12 agents in All, got %d", len(result.All))
	}
}
