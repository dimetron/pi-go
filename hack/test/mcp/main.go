...
func rootsTool(ctx context.Context, req *mcp.CallToolRequest, _ any) (*mcp.CallToolResult, any, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("listing roots failed: %w", err)
	}
	var allroots []string
...
func elicitFormTool(ctx context.Context, req *mcp.CallToolRequest, _ any) (*mcp.CallToolResult, any, error) {
	...
	randomStr, ok := res.Content["random"].(string)
	if !ok {
		return nil, nil, fmt.Errorf("eliciting returned unexpected type")
	}
	return &mcp.CallToolResult{
		Content: []mcp.Content{
			&mcp.TextContent{Text: randomStr},
		},
	}, nil, nil
}
