...
Or run vet directly if golangci-lint is not available:
...
- [ ] `make lint` exits with code 0 (no issues)
- [ ] `go vet ./...` exits with code 0 (no issues)
...
- Always run `make lint` first (most comprehensive)
- Fall back to `go vet ./...` if golangci-lint fails to run
